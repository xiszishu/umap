...
static int test_openfiles(const char *fn)
{
    ...
    bk_list = umt_openandmap_fits2p(&options,totalbytes,base_addr,(off_t)dstart,frame);
    ...
    umt_closeandunmap_fits2p(&options, totalbytes, base_addr, bk_list);
    free(cube_median);
    free(list);
    return 0 ;
}
