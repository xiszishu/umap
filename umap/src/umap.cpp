...
const int NUM_PAGE_CACHE_SETS = 256;

static long page_size;

inline static int64_t int64_hash(int64_t key)
{
...
class _umap {
    public:
        ...
        // }
        inline sa_index get_page_index(void* _p) 
	{
	    int id_set=int64_hash((int64_t)_p) % NUM_PAGE_CACHE_SETS;
	    if (id_set<0) id_set*=-1;
            auto it = page_index.find(_p);
            return (it == page_index.end()) ? (struct sa_index){id_set,-1} : it->second;
        }
    ...
    private:
        ...
        int userfault_fd;
        int next_page_alloc_index[NUM_PAGE_CACHE_SETS];
        thread *listener;
        vector<umap_page> pages_in_memory[NUM_PAGE_CACHE_SETS];
        char* tmppagebuf;
        
        int set_size;

        map<void*, sa_index> page_index;
        ...
        inline void add_page_index(sa_index idx, void* page) {
            page_index[page] = idx;
        }
...
_umap::_umap(void* _mmap_addr, size_t _mmap_length,int num_backing_file,umap_backing_file* backing_files)
    ...
    time_to_stop{false}, fault_count{0}
{
    for (int i=0;i<num_backing_file;i++)
        bk_files.push_back(backing_files[i]); 
    page_buffer_size = umap_page_bufsize;
    set_size= page_buffer_size/NUM_PAGE_CACHE_SETS;
    memset(next_page_alloc_index,0,sizeof(next_page_alloc_index));
    ...
    for (int i=0;i<=NUM_PAGE_CACHE_SETS;i++)
    pages_in_memory[i].resize(page_buffer_size/NUM_PAGE_CACHE_SETS,ump);
...
void _umap::pagefault_event(const struct uffd_msg& msg)
{
    ...
    if (id.index>=0) {
        if (msg.arg.pagefault.flags & (UFFD_PAGEFAULT_FLAG_WP | UFFD_PAGEFAULT_FLAG_WRITE)) {
            ...
            if ((msg.arg.pagefault.flags & (UFFD_PAGEFAULT_FLAG_WP | UFFD_PAGEFAULT_FLAG_WRITE)) == (UFFD_PAGEFAULT_FLAG_WP | UFFD_PAGEFAULT_FLAG_WRITE))
                umapdbg("PF(WP+WRITE) (In Memory Already) @(%p)=%lu %s\n", page_begin, *(uint64_t*)page_begin, pages_in_memory[id.set_id][id.index].page_is_dirty() ? "Already Dirty" : "Clean");
            ...
            pages_in_memory[id.set_id][id.index].mark_page_dirty();
    ...
    if (pages_in_memory[id.set_id][next_page_alloc_index[id.set_id]].get_page()) {
        delete_page_index(pages_in_memory[id.set_id][next_page_alloc_index[id.set_id]].get_page());
        evict_page(pages_in_memory[id.set_id][next_page_alloc_index[id.set_id]]);
    }
    pages_in_memory[id.set_id][next_page_alloc_index[id.set_id]].set_page(page_begin);
    add_page_index((struct sa_index){id.set_id,next_page_alloc_index[id.set_id]}, page_begin);
    ...
    if (msg.arg.pagefault.flags & (UFFD_PAGEFAULT_FLAG_WP | UFFD_PAGEFAULT_FLAG_WRITE)) {
        pages_in_memory[id.set_id][next_page_alloc_index[id.set_id]].mark_page_dirty();
        ...
        pages_in_memory[id.set_id][next_page_alloc_index[id.set_id]].mark_page_clean();
    ...
    next_page_alloc_index[id.set_id] = (next_page_alloc_index[id.set_id] +1) % set_size;
}
...
void sighandler(int signum, siginfo_t *info, void* buf)
{
    ...
    for (auto it : active_umaps) {
        if (it.second->is_in_umap(page_begin)) {
            num_bus_errs++;
#ifdef DEBUG
            if (it.second->get_page_index(page_begin).index >= 0)
